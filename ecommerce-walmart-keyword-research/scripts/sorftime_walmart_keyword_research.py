#!/usr/bin/env python3
"""
Walmart Keyword Research - Nexscope Skill
Call the /sorftime/walmart/keywordResearch endpoint.

Usage:
  python sorftime_walmart_keyword_research.py '<JSON parameters>'           # Always save results; print small results in full and summarize large results
  python sorftime_walmart_keyword_research.py '<JSON parameters>' --inline  # Force full output to stdout

Output policy (default script behavior):
  - **Always** save the full response to `<cwd>/nexscope/<YYYY-MM-DD>/<session>/data/nexscope-sorftime-walmart-keyword-research-<timestamp>.json` by default; SESSION_ID groups outputs by user task. If the workspace is not writable, use the fallback roots documented in _nexscope_root().
...
API_PATH = "/api/v1/tools/research/sorftime/walmart/keywordResearch"
SLUG = "ecommerce.walmart-keyword-research"
...
def validate_public_operation(params):
    if not isinstance(params, dict):
        raise ValueError("Parameters must be a JSON object")
    operation = params.get("operation")
    if operation not in PUBLIC_OPERATIONS:
        raise ValueError(
            "Only public read operations are supported; favoriteAdd and "
            "favoriteChange are intentionally excluded"
        )
...
def call_api(params):
    validate_public_operation(params)
    api_url = get_api_url()
    api_key = get_api_key()
