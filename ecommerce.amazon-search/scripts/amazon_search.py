#!/usr/bin/env python3
"""
NexScope proxy client for /api/v1/tools/research/amazon/search.

Usage:
  python amazon_search.py '<JSON parameters>'
  python amazon_search.py '<JSON parameters>' --inline    # force full stdout output
  python amazon_search.py '<JSON parameters>' --no-cache  # skip local cache

Output strategy (default script behavior):
  - Always writes full response to <cwd>/nexscope/<YYYY-MM-DD>/<session>/data/ecommerce.amazon-search-amazon_search-<timestamp>.json
...
API_PATH = '/api/v1/tools/research/amazon/search'
SLUG = 'ecommerce.amazon-search-amazon_search'
