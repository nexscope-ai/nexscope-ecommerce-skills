...
输出策略（脚本默认行为）：
  - **始终**将完整响应写入 `<cwd>/nexscope/<YYYY-MM-DD>/<session>/data/nexscope-1688-product-detail-<timestamp>.json`（`<cwd>` 为脚本执行时的工作目录，在 Claude Code 里即当前项目目录；`<session>` 取自环境变量 `SESSION_ID`，按用户任务自动聚合；**禁止写入 /tmp**，当前目录不可写则报错）
