#!/usr/bin/env python3
"""
1688 Product Detail - Nexscope Skill
Call the /alibaba1688/productDetail endpoint.

Usage:
  python alibaba1688_product_detail.py '<JSON parameters>'           # Automatic: print small results in full; save large results and print a summary
  python alibaba1688_product_detail.py '<JSON parameters>' --inline  # Force full output to stdout

Output policy (default script behavior):
  - **Always** save the full response to `<cwd>/nexscope/<YYYY-MM-DD>/<session>/data/nexscope-1688-product-detail-<timestamp>.json` by default; SESSION_ID groups outputs by user task. If the workspace is not writable, use the fallback roots documented in _nexscope_root().
...
API_PATH = "/api/v1/tools/research/alibaba1688/productDetail"
SLUG = "ecommerce.1688-product-detail"
