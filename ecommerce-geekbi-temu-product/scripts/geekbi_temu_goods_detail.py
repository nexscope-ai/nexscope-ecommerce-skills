...
Call the /api/v1/tools/research/geekbi/temu/goodsDetail endpoint.

Usage:
  python geekbi_temu_goods_detail.py '<JSON parameters>'
  python geekbi_temu_goods_detail.py '<JSON parameters>' --inline
...
CONTRACT_JSON = """
{
  ...
  "properties": {
    "goodsId": {
      "type": "string",
      "description": "Product ID returned by product search.",
      "minLength": 1,
      "maxLength": 100
    },
    "regionId": {
...
def main():
    ...
    if not argv:
        print(
            "Usage: geekbi_temu_goods_detail.py '<JSON parameters>' [--inline] [--no-cache]",
            file=sys.stderr,
        )
        sys.exit(1)
