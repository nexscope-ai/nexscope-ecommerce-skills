#!/usr/bin/env python3
"""
Google Patents Search - Nexscope Skill
Call the /googlePatent/search endpoint.

Usage:
  python google_patent_search.py '<JSON parameters>'           # Automatic: print small results in full; save large results and print a summary
  python google_patent_search.py '<JSON parameters>' --inline  # Force full output to stdout

Output policy (default script behavior):
  - **Always** save the full response to `<cwd>/nexscope/<YYYY-MM-DD>/<session>/data/nexscope-google-patent-search-<timestamp>.json` by default; SESSION_ID groups outputs by user task. If the workspace is not writable, use the fallback roots documented in _nexscope_root().
...
API_PATH = "/api/v1/tools/research/googlePatent/search"
SLUG = "ecommerce.google-patent-search"
