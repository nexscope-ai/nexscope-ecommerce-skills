#!/usr/bin/env python3
"""
TikTok Most-Promoted Product Ranking - Nexscope Skill
Call the /api/v1/tools/research/chuhaijiang/products/rankings/most-promoted endpoint.

Usage:
  python chuhaijiang_product_rank_most_promoted.py '<JSON parameters>'           # Automatic: full small results; save large results and print a summary
  python chuhaijiang_product_rank_most_promoted.py '<JSON parameters>' --inline  # Force full output to stdout
...
API_PATH = "/api/v1/tools/research/chuhaijiang/products/rankings/most-promoted"
...
def validate_params(params):
    ...
    rules = {'country': {'type': 'string',
     ...
     'sort': {'type': 'string',
              'pattern': '^(interval_gmv|interval_sold_count|live_count|live_user_count|related_creator_count|video_play_count):(asc|desc)$'}}
...
def main():
    ...
    if len(argv) != 1:
        print(
            "Usage: chuhaijiang_product_rank_most_promoted.py '<JSON parameters>' [--inline] [--no-cache]",
            file=sys.stderr,
        )
        sys.exit(1)
