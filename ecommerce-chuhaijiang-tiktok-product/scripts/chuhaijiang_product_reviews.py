#!/usr/bin/env python3
"""
TikTok Product Reviews - Nexscope Skill
Call the /api/v1/tools/research/chuhaijiang/products/reviews endpoint.

Usage:
  python chuhaijiang_product_reviews.py '<JSON parameters>'           # Automatic: full small results; save large results and print a summary
  python chuhaijiang_product_reviews.py '<JSON parameters>' --inline  # Force full output to stdout
...
API_PATH = "/api/v1/tools/research/chuhaijiang/products/reviews"
...
def main():
    ...
    if len(argv) != 1:
        print(
            "Usage: chuhaijiang_product_reviews.py '<JSON parameters>' [--inline] [--no-cache]",
            file=sys.stderr,
        )
        sys.exit(1)
