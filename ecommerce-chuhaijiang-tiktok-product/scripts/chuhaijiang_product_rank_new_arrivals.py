#!/usr/bin/env python3
"""
TikTok New-Arrival Product Ranking - Nexscope Skill
Call the /api/v1/tools/research/chuhaijiang/products/rankings/new-arrivals endpoint.

Usage:
  python chuhaijiang_product_rank_new_arrivals.py '<JSON parameters>'           # Automatic: full small results; save large results and print a summary
  python chuhaijiang_product_rank_new_arrivals.py '<JSON parameters>' --inline  # Force full output to stdout
...
API_PATH = "/api/v1/tools/research/chuhaijiang/products/rankings/new-arrivals"
...
def validate_params(params):
    ...
    rules = {'country': {'type': 'string',
     ...
     'sort': {'type': 'string',
              'pattern': '^(gmv_3d|sold_count_3d|total_gmv|total_sold_count):(asc|desc)$'},
     'listedFrom': {'type': 'string', 'pattern': '^[0-9]{8}$'},
     'listedTo': {'type': 'string', 'pattern': '^[0-9]{8}$'},
     'productStatus': {'type': 'string', 'enum': ['1', '2', '3', '4']}}
    required = ('country',)
...
def main():
    ...
    if len(argv) != 1:
        print(
            "Usage: chuhaijiang_product_rank_new_arrivals.py '<JSON parameters>' [--inline] [--no-cache]",
            file=sys.stderr,
        )
        sys.exit(1)
