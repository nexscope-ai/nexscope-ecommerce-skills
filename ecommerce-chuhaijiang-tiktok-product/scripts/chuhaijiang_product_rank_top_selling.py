#!/usr/bin/env python3
"""
TikTok Top-Selling Product Ranking - Nexscope Skill
Call the /api/v1/tools/research/chuhaijiang/products/rankings/top-selling endpoint.

Usage:
  python chuhaijiang_product_rank_top_selling.py '<JSON parameters>'           # Automatic: full small results; save large results and print a summary
  python chuhaijiang_product_rank_top_selling.py '<JSON parameters>' --inline  # Force full output to stdout
...
API_PATH = "/api/v1/tools/research/chuhaijiang/products/rankings/top-selling"
...
def validate_params(params):
    ...
    rules = {'country': {'type': 'string',
     ...
     'sort': {'type': 'string',
              'pattern': '^(gmv_growth_rate|interval_gmv|interval_sold_count|sold_count_growth_rate|total_gmv|total_sold_count):(asc|desc)$'}}
...
def main():
    ...
    if len(argv) != 1:
        print(
            "Usage: chuhaijiang_product_rank_top_selling.py '<JSON parameters>' [--inline] [--no-cache]",
            file=sys.stderr,
        )
        sys.exit(1)
