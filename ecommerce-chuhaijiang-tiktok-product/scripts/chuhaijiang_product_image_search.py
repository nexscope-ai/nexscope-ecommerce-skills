#!/usr/bin/env python3
"""
TikTok Product Image Search - Nexscope Skill
Call the /api/v1/tools/research/chuhaijiang/products/image-search endpoint.

Usage:
  python chuhaijiang_product_image_search.py '<JSON parameters>'           # Automatic: full small results; save large results and print a summary
  python chuhaijiang_product_image_search.py '<JSON parameters>' --inline  # Force full output to stdout
...
API_PATH = "/api/v1/tools/research/chuhaijiang/products/image-search"
...
def validate_params(params):
    ...
    rules = {'osKey': {'type': 'string'}, 'country': {'type': 'string', 'pattern': '^[A-Z]{2}$'}}
    required = ('osKey',)
...
def main():
    ...
    if len(argv) != 1:
        print(
            "Usage: chuhaijiang_product_image_search.py '<JSON parameters>' [--inline] [--no-cache]",
            file=sys.stderr,
        )
        sys.exit(1)
