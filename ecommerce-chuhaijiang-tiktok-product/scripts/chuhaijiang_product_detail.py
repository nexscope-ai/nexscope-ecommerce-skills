#!/usr/bin/env python3
"""
TikTok Product Detail - Nexscope Skill
Call the /api/v1/tools/research/chuhaijiang/products/detail endpoint.

Usage:
  python chuhaijiang_product_detail.py '<JSON parameters>'           # Automatic: full small results; save large results and print a summary
  python chuhaijiang_product_detail.py '<JSON parameters>' --inline  # Force full output to stdout
...
API_PATH = "/api/v1/tools/research/chuhaijiang/products/detail"
...
def validate_params(params):
    ...
    rules = {'country': {'type': 'string',
                 'enum': ['br',
                          ...
                          'vn']},
     'id': {'type': 'string'},
     'include': {'type': 'string', 'pattern': '^(core|channel)(,(core|channel))*$'}}
    required = ('country', 'id')
...
def main():
    ...
    if len(argv) != 1:
        print(
            "Usage: chuhaijiang_product_detail.py '<JSON parameters>' [--inline] [--no-cache]",
            file=sys.stderr,
        )
        sys.exit(1)
