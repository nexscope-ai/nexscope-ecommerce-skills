#!/usr/bin/env python3
"""
NexScope proxy client for /api/v1/tools/research/zhihuiya/simpleBibliography.

Usage:
  python patent_core_bibliography.py '<JSON parameters>'
  python patent_core_bibliography.py '<JSON parameters>' --inline    # force full stdout output
  python patent_core_bibliography.py '<JSON parameters>' --no-cache  # skip local cache

Output strategy (default script behavior):
  - Always writes full response to <cwd>/nexscope/<YYYY-MM-DD>/<session>/data/ecommerce-patent-core-bibliography-patent_core_bibliography-<timestamp>.json
...
API_PATH = '/api/v1/tools/research/zhihuiya/simpleBibliography'
SLUG = 'ecommerce-patent-core-bibliography-patent_core_bibliography'
...
def record_output(out_path):
    session_dir = os.path.dirname(os.path.dirname(out_path)); root = os.path.dirname(os.path.dirname(session_dir)); meta_path = os.path.join(session_dir, "_meta.json"); created = not os.path.exists(meta_path)
    try:
        if created: meta = {"session_id": os.path.basename(session_dir), "date": os.path.basename(os.path.dirname(session_dir)), "started_at": time.strftime("%Y-%m-%dT%H:%M:%S%z"), "skills_called": [], "deliverables": [], "data_files": [], "media_files": []}
        else:
            with open(meta_path, encoding="utf-8") as f: meta = json.load(f)
        if SLUG not in meta.setdefault("skills_called", []): meta["skills_called"].append(SLUG)
        rel = os.path.relpath(out_path, session_dir)
        if rel not in meta.setdefault("data_files", []): meta["data_files"].append(rel)
        meta["last_used_at"] = time.strftime("%Y-%m-%dT%H:%M:%S%z")
        with open(meta_path, "w", encoding="utf-8") as f: json.dump(meta, f, ensure_ascii=False, indent=2)
        if created:
            with open(os.path.join(root, "index.jsonl"), "a", encoding="utf-8") as f: f.write(json.dumps({"session_id": meta["session_id"], "date": meta["date"], "path": os.path.relpath(session_dir, root), "started_at": meta["started_at"]}, ensure_ascii=False) + "\n")
