#!/usr/bin/env python3
"""
EchoTik - TikTok Video Ranking - Nexscope Skill
Call the /echotik/listVideoRank endpoint.

Usage:
  python echotik_list_video_rank.py '<JSON parameters>'           # Automatic: print small results in full; save large results and print a summary
  python echotik_list_video_rank.py '<JSON parameters>' --inline  # Force full output to stdout

Output policy (default script behavior):
  - **Always** save the full response to `<cwd>/nexscope/<YYYY-MM-DD>/<session>/data/nexscope-echotik-list-video-rank-<timestamp>.json` by default; SESSION_ID groups outputs by user task. If the workspace is not writable, use the fallback roots documented in _nexscope_root().
...
API_PATH = "/api/v1/tools/research/echotik/listVideoRank"
SLUG = "ecommerce.tiktok-video-rank"
