...
- **Python Scripts**: `python scripts/product_rank.py '<JSON parameters>' [--inline]` or `python scripts/product_detail.py '<JSON parameters>' [--inline]`
...
```text
Run `product_rank.py` first, choose a row's `product_id`, then pass that value as `productId` to `product_detail.py`.
```
...
- A valid-but-empty request (e.g. an unsupported `region`) may return outer `code: 0` with an empty business payload and still be billed.
