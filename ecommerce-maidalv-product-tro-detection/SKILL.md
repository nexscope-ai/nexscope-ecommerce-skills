...
|---|---|---:|---:|
| `maidalv_check_api_flash` | local-script | no | yes |
| `upload_image` | local-script | yes | yes |
...
`mainProductImage` is required. For local images, run `scripts/upload_image.py` first. Read `references/api.md` for all optional evidence inputs and risk-result fields.
