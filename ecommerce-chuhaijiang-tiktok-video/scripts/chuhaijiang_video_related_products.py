#!/usr/bin/env python3
"""
TikTok Video Related Products - Nexscope Skill
Call /api/v1/tools/research/chuhaijiang/videos/related-products endpoint.

Usage:
  python chuhaijiang_video_related_products.py '<JSON parameters>'           # Automatic: full small results; save large results to a file + summary
  python chuhaijiang_video_related_products.py '<JSON parameters>' --inline  # Force complete output to stdout

Output policy (default script behavior):
  - **Always** write the complete response to `<cwd>/nexscope/<YYYY-MM-DD>/<session>/data/ecommerce.chuhaijiang-tiktok-video-<timestamp>.json` (`<cwd>` is the working directory when the script runs, which is the current project directory in Claude Code; `<session>` comes from the `SESSION_ID` environment variable to group outputs automatically by user task; **never write to /tmp**; report an error if the current directory is not writable)
  - Response body <= 8 KB: save to disk, then print the complete JSON to stdout
  - Response body > 8 KB: save to disk, then print only a summary to stdout (top-level fields, common counts such as `total`/`count`, length of the largest list field + first 3 samples)
...
API_PATH = "/api/v1/tools/research/chuhaijiang/videos/related-products"
SLUG = "ecommerce.chuhaijiang-tiktok-video"
...
def _find_main_list(obj):
    """Recursively find the list field with the most elements, without hard-coded field names, for any structure."""
    best = (None, None, -1)
...
def main():
    ...
    if not argv:
        print(
            "Usage: chuhaijiang_video_related_products.py '<JSON parameters>' [--inline] [--no-cache]",
            file=sys.stderr,
        )
        sys.exit(1)
