#!/usr/bin/env python3
"""
TikTok Video Reviews - Nexscope Skill
Call /api/v1/tools/research/chuhaijiang/videos/reviews endpoint.

Usage:
  python chuhaijiang_video_reviews.py '<JSON parameters>'           # Automatic: full small results; save large results to a file + summary
  python chuhaijiang_video_reviews.py '<JSON parameters>' --inline  # Force complete output to stdout
...
API_PATH = "/api/v1/tools/research/chuhaijiang/videos/reviews"
...
def main():
    ...
    if not argv:
        print(
            "Usage: chuhaijiang_video_reviews.py '<JSON parameters>' [--inline] [--no-cache]",
            file=sys.stderr,
        )
        sys.exit(1)
