...
### Endpoint
```
POST {NEXSCOPE_PROXY_BASE}/api/v1/tools/research/googleTrend/getTrendByKeys
```
...
### Endpoint
```
POST {NEXSCOPE_PROXY_BASE}/api/v1/tools/research/amazon/search
```
...
### Endpoint
```
POST {NEXSCOPE_PROXY_BASE}/api/v1/tools/research/echotik/listProduct
```
...
### Endpoint
```
POST {NEXSCOPE_PROXY_BASE}/api/v1/tools/research/keepa/productSeries
```
...
### Endpoint
```
POST {NEXSCOPE_PROXY_BASE}/api/v1/tools/research/keepa/productRequest
```
...
### Endpoint
```
POST {NEXSCOPE_PROXY_BASE}/api/v1/tools/research/amazon/product/detail
```
...
# 2. Google Trends
curl -s -X POST "{NEXSCOPE_PROXY_BASE}/api/v1/tools/research/googleTrend/getTrendByKeys" \
...
# 3. Amazon Search
curl -s -X POST "{NEXSCOPE_PROXY_BASE}/api/v1/tools/research/amazon/search" \
  -H "Authorization: Bearer ${NEXSCOPE_API_KEY}" -H "Content-Type: application/json" \
  -d "{\"keyword\":\"$KEYWORD\",\"amazonDomain\":\"amazon.com\",\"page\":1}" \
  > /tmp/amz_search.json

# 4. TikTok
curl -s -X POST "{NEXSCOPE_PROXY_BASE}/api/v1/tools/research/echotik/listProduct" \
  -H "Authorization: Bearer ${NEXSCOPE_API_KEY}" -H "Content-Type: application/json" \
...
curl -s -X POST "{NEXSCOPE_PROXY_BASE}/api/v1/tools/research/keepa/productSeries" \
  -H "Authorization: Bearer ${NEXSCOPE_API_KEY}" -H "Content-Type: application/json" \
  -d "{\"asin\":\"$TOP_ASIN\",\"domain\":\"1\",\"days\":90,\"showBsrMain\":1,\"showSellerCount\":1,\"showPrice\":1}" \
  > /tmp/keepa_hist.json

# 7. Keepa Detail (batch)
curl -s -X POST "{NEXSCOPE_PROXY_BASE}/api/v1/tools/research/keepa/productRequest" \
  -H "Authorization: Bearer ${NEXSCOPE_API_KEY}" -H "Content-Type: application/json" \
  -d "{\"asin\":\"$TOP_5_ASINS\",\"domain\":\"1\",\"history\":1}" \
  > /tmp/keepa_detail.json

# 8. Amazon Detail (top ASIN with reviews)
curl -s -X POST "{NEXSCOPE_PROXY_BASE}/api/v1/tools/research/amazon/product/detail" \
  -H "Authorization: Bearer ${NEXSCOPE_API_KEY}" -H "Content-Type: application/json" \
