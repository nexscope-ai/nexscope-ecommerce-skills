...
if "--help" in _nexscope_help_sys.argv or "-h" in _nexscope_help_sys.argv:
    print('Usage: python aigc_videogen_multi.py [arguments]')
    raise SystemExit(0)
...
CREATE_PATH = "/api/v1/tools/research/aigc/multiImageVideoGenAsync"
QUERY_PATH = "/api/v1/tools/research/aigc/taskQuery"
SLUG = "nexscope-aigc-videogen-multi"
POLL_INTERVAL = 10
MAX_POLL_TIME = 1200
INITIAL_DELAY = 120
...
def main():
    ...
    required = ("imageList", "videoType", "videoTime", "prompt")
    missing = [key for key in required if params.get(key) in (None, "", [])]
    if missing:
        sys.exit("Missing required parameters: " + ", ".join(missing))
