---
name: ecommerce.aigc-videogen-multi
version: 1.0.0
category: ecommerce
description: Generate video asynchronously from multiple reference images and a prompt. Use this skill when the user asks for this exact ecommerce workflow, analysis, data lookup, or deliverable.
---
...
Generate video asynchronously from multiple reference images and a prompt.
...
|---|---|---:|---:|
| `aigc_videogen_multi` | local-script | yes | yes |
| `aigc_task_query` | local-script | no | yes |
...
The create operation consumes credits and creates external state. Require an explicit prompt, retain the returned `taskId`, and query that task after ambiguous timeouts instead of creating another one. Read `references/api.md` for model-specific limits and polling behavior.
