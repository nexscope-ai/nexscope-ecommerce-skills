#!/usr/bin/env python3
"""
Walmart Category Market Research - Nexscope Skill
Call the /sorftime/walmart/categoryMarket endpoint.

Usage:
  python sorftime_walmart_category_market.py '<JSON parameters>'           # Always save results; print small results in full and summarize large results
  python sorftime_walmart_category_market.py '<JSON parameters>' --inline  # Force full output to stdout

Output policy (default script behavior):
  - **Always** save the full response to `<cwd>/nexscope/<YYYY-MM-DD>/<session>/data/nexscope-sorftime-walmart-category-market-<timestamp>.json` by default; SESSION_ID groups outputs by user task. If the workspace is not writable, use the fallback roots documented in _nexscope_root().
...
API_PATH = "/api/v1/tools/research/sorftime/walmart/categoryMarket"
SLUG = "ecommerce.walmart-category-market"
