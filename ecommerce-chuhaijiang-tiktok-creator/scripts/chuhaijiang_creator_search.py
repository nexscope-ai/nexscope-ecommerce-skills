#!/usr/bin/env python3
"""
TikTok Creator Search - Nexscope Skill
Call the /api/v1/tools/research/chuhaijiang/creators/search endpoint.

Usage:
  python chuhaijiang_creator_search.py '<JSON parameters>'           # Automatic: full small results; save large results and print a summary
  python chuhaijiang_creator_search.py '<JSON parameters>' --inline  # Force full output to stdout
...
API_PATH = "/api/v1/tools/research/chuhaijiang/creators/search"
...
def validate_params(params):
    ...
    rules = {'country': {'type': 'string',
     ...
     'minGmv30d': {'type': 'number'},
     'maxGmv30d': {'type': 'number'},
     'minAvgViews': {'type': 'number'},
     'maxAvgViews': {'type': 'number'},
...
def main():
    ...
    if len(argv) != 1:
        print(
            "Usage: chuhaijiang_creator_search.py '<JSON parameters>' [--inline] [--no-cache]",
            file=sys.stderr,
        )
        sys.exit(1)
