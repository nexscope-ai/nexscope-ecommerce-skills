#!/usr/bin/env python3
"""
TikTok Creator Related Livestreams - Nexscope Skill
Call the /api/v1/tools/research/chuhaijiang/creators/related-lives endpoint.

Usage:
  python chuhaijiang_creator_related_lives.py '<JSON parameters>'           # Automatic: full small results; save large results and print a summary
  python chuhaijiang_creator_related_lives.py '<JSON parameters>' --inline  # Force full output to stdout
...
API_PATH = "/api/v1/tools/research/chuhaijiang/creators/related-lives"
...
def main():
    ...
    if len(argv) != 1:
        print(
            "Usage: chuhaijiang_creator_related_lives.py '<JSON parameters>' [--inline] [--no-cache]",
            file=sys.stderr,
        )
        sys.exit(1)
