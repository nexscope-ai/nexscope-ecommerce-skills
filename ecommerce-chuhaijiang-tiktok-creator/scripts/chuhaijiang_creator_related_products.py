#!/usr/bin/env python3
"""
TikTok Creator Related Products - Nexscope Skill
Call the /api/v1/tools/research/chuhaijiang/creators/related-products endpoint.

Usage:
  python chuhaijiang_creator_related_products.py '<JSON parameters>'           # Automatic: full small results; save large results and print a summary
  python chuhaijiang_creator_related_products.py '<JSON parameters>' --inline  # Force full output to stdout
...
API_PATH = "/api/v1/tools/research/chuhaijiang/creators/related-products"
...
def main():
    ...
    if len(argv) != 1:
        print(
            "Usage: chuhaijiang_creator_related_products.py '<JSON parameters>' [--inline] [--no-cache]",
            file=sys.stderr,
        )
        sys.exit(1)
