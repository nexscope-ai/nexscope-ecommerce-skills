#!/usr/bin/env python3
"""
TikTok Creator Detail - Nexscope Skill
Call the /api/v1/tools/research/chuhaijiang/creators/detail endpoint.

Usage:
  python chuhaijiang_creator_detail.py '<JSON parameters>'           # Automatic: full small results; save large results and print a summary
  python chuhaijiang_creator_detail.py '<JSON parameters>' --inline  # Force full output to stdout
...
API_PATH = "/api/v1/tools/research/chuhaijiang/creators/detail"
...
def validate_params(params):
    ...
    rules = {'country': {'type': 'string',
     ...
     'id': {'type': 'string'},
     'include': {'type': 'string',
                 'pattern': '^(core|channel|portrait)(,(core|channel|portrait))*$'}}
    required = ('country', 'id')
...
def main():
    ...
    if len(argv) != 1:
        print(
            "Usage: chuhaijiang_creator_detail.py '<JSON parameters>' [--inline] [--no-cache]",
            file=sys.stderr,
        )
        sys.exit(1)
