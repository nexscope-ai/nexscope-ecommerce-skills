#!/usr/bin/env python3
"""
TikTok Creator Growth Ranking - Nexscope Skill
Call the /api/v1/tools/research/chuhaijiang/creators/rankings/growth endpoint.

Usage:
  python chuhaijiang_creator_rank_growth.py '<JSON parameters>'           # Automatic: full small results; save large results and print a summary
  python chuhaijiang_creator_rank_growth.py '<JSON parameters>' --inline  # Force full output to stdout

Output policy (default script behavior):
  - **Always** write the complete response to `<nexscope-root>/<YYYY-MM-DD>/<session>/data/ecommerce.chuhaijiang-tiktok-creator-<timestamp>.json` (`<cwd>` is the working directory when the script runs; `<session>` comes from the `SESSION_ID` environment variable)
...
API_PATH = "/api/v1/tools/research/chuhaijiang/creators/rankings/growth"
SLUG = "ecommerce.chuhaijiang-tiktok-creator"
...
def validate_params(params):
    ...
    rules = {'country': {'type': 'string',
     ...
     'granularity': {'type': 'string',
                     'enum': ['daily', 'weekly', 'monthly', '0', '1', '2']},
     'sort': {'type': 'string', 'pattern': '^[A-Za-z0-9_]+:(asc|desc)$'},
     'creatorCategory': {'type': 'string'},
     'productCategory': {'type': 'string'}}
    required = ('country', 'date', 'granularity')
...
def main():
    ...
    if len(argv) != 1:
        print(
            "Usage: chuhaijiang_creator_rank_growth.py '<JSON parameters>' [--inline] [--no-cache]",
            file=sys.stderr,
        )
        sys.exit(1)
