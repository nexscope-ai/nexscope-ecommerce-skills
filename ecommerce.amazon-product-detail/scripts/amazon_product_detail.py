#!/usr/bin/env python3
"""
NexScope proxy client for /api/v1/tools/research/amazon/product/detail.

Usage:
  python amazon_product_detail.py '<JSON parameters>'
  python amazon_product_detail.py '<JSON parameters>' --inline    # force full stdout output
  python amazon_product_detail.py '<JSON parameters>' --no-cache  # skip local cache

Output strategy (default script behavior):
  - Always writes full response to <cwd>/nexscope/<YYYY-MM-DD>/<session>/data/ecommerce.amazon-product-detail-amazon_product_detail-<timestamp>.json
...
SLUG = 'ecommerce.amazon-product-detail-amazon_product_detail'
