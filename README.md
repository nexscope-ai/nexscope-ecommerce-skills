...
Preview every available skill with the Skills CLI:

```bash
npx skills add nexscope-ai/nexscope-ecommerce-skills --list
```
...
Each top-level `ecommerce-*` directory is an individual skill package. You can also copy a complete directory into the skills location used by your assistant. Keep its `SKILL.md`, scripts, references, assets, and manifest files together.
...
> **Naming update:** Earlier clones used dotted names such as `ecommerce.amazon-search`. Current Agent Skills-compatible names use hyphens, such as `ecommerce-amazon-search`. Replace old dotted directories instead of keeping both copies.
