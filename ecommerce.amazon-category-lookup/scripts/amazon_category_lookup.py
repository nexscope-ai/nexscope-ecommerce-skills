...
if "--help" in _nexscope_help_sys.argv or "-h" in _nexscope_help_sys.argv:
    print('Usage: python amazon_category_lookup.py [arguments]')
    raise SystemExit(0)
...
API_PATH_LOOKUP = "/api/v1/tools/research/amazon/nodes/lookup"
API_PATH_LIKE = "/api/v1/tools/research/amazon/nodes/lookup/like"
SLUG = "nexscope-amazon-category-lookup"
...
def main():
    ...
    if not isinstance(params, dict):
        sys.exit("Parameters must be a JSON object")
    if mode == "like" and not params.get("nodeLabel"):
        sys.exit("Missing required parameter for like mode: nodeLabel")
