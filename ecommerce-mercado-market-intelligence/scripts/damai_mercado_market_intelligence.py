#!/usr/bin/env python3
"""
Damai Mercado Libre Market Intelligence - Nexscope Skill
Call the /damai/call endpoint.

Usage:
  python damai_mercado_market_intelligence.py '<JSON parameters>'           # Automatic: print small results in full; save large results and print a summary
  python damai_mercado_market_intelligence.py '<JSON parameters>' --inline  # Force full output to stdout

Output policy (default script behavior):
  - **Always** save the full response to `<cwd>/nexscope/<YYYY-MM-DD>/<session>/data/nexscope-damai-mercado-market-intelligence-<timestamp>.json` by default; SESSION_ID groups outputs by user task. If the workspace is not writable, use the fallback roots documented in _nexscope_root().
...
API_PATH = "/api/v1/tools/research/damai/call"
SLUG = "ecommerce.mercado-market-intelligence"
