...
Usage:
  python chuhaijiang_ad_related_products.py '<JSON parameters>'           # Automatic: full small results; save large results to a file + summary
  python chuhaijiang_ad_related_products.py '<JSON parameters>' --inline  # Force complete output to stdout
...
API_PATH = "/api/v1/tools/research/chuhaijiang/ad-creative/ads/related-products"
...
def main():
    ...
    if not argv:
        print(
            "Usage: chuhaijiang_ad_related_products.py '<JSON parameters>' [--inline] [--no-cache]",
            file=sys.stderr,
        )
        sys.exit(1)
