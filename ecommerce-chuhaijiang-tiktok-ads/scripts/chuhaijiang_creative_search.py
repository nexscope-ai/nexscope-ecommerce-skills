...
Call /api/v1/tools/research/chuhaijiang/ad-creative/creatives/search endpoint.

Usage:
  python chuhaijiang_creative_search.py '<JSON parameters>'           # Automatic: full small results; save large results to a file + summary
  python chuhaijiang_creative_search.py '<JSON parameters>' --inline  # Force complete output to stdout
...
API_PATH = "/api/v1/tools/research/chuhaijiang/ad-creative/creatives/search"
...
def main():
    ...
    if not argv:
        print(
            "Usage: chuhaijiang_creative_search.py '<JSON parameters>' [--inline] [--no-cache]",
            file=sys.stderr,
        )
        sys.exit(1)
