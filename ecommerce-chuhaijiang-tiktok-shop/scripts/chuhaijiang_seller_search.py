#!/usr/bin/env python3
"""
TikTok Shop Store Search - Nexscope Skill
Call the /api/v1/tools/research/chuhaijiang/sellers/search endpoint.

Usage:
  python chuhaijiang_seller_search.py '<JSON parameters>'           # Automatic: full small results; save large results and print a summary
  python chuhaijiang_seller_search.py '<JSON parameters>' --inline  # Force full output to stdout
...
API_PATH = "/api/v1/tools/research/chuhaijiang/sellers/search"
...
def validate_params(params):
    ...
    rules = {'country': {'type': 'string',
     ...
     'minGmv7d': {'type': 'number'},
     'maxGmv7d': {'type': 'number'}}
    required = ('country',)
...
def main():
    ...
    if not argv:
        print(
            "Usage: chuhaijiang_seller_search.py '<JSON parameters>' [--inline] [--no-cache]",
            file=sys.stderr,
        )
        sys.exit(1)
