#!/usr/bin/env python3
"""
TikTok Shop Store Related Products - Nexscope Skill
Call the /api/v1/tools/research/chuhaijiang/sellers/related-products endpoint.

Usage:
  python chuhaijiang_seller_related_products.py '<JSON parameters>'           # Automatic: full small results; save large results and print a summary
  python chuhaijiang_seller_related_products.py '<JSON parameters>' --inline  # Force full output to stdout
...
API_PATH = "/api/v1/tools/research/chuhaijiang/sellers/related-products"
...
def main():
    ...
    if not argv:
        print(
            "Usage: chuhaijiang_seller_related_products.py '<JSON parameters>' [--inline] [--no-cache]",
            file=sys.stderr,
        )
        sys.exit(1)
