#!/usr/bin/env python3
"""
TikTok Shop Store Detail - Nexscope Skill
Call the /api/v1/tools/research/chuhaijiang/sellers/detail endpoint.

Usage:
  python chuhaijiang_seller_detail.py '<JSON parameters>'           # Automatic: full small results; save large results and print a summary
  python chuhaijiang_seller_detail.py '<JSON parameters>' --inline  # Force full output to stdout
...
API_PATH = "/api/v1/tools/research/chuhaijiang/sellers/detail"
...
def main():
    ...
    if not argv:
        print(
            "Usage: chuhaijiang_seller_detail.py '<JSON parameters>' [--inline] [--no-cache]",
            file=sys.stderr,
        )
        sys.exit(1)
