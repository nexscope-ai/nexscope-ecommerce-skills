#!/usr/bin/env python3
"""
TikTok Shop Top-Selling Store Ranking - Nexscope Skill
Call the /api/v1/tools/research/chuhaijiang/sellers/rankings/top-selling endpoint.

Usage:
  python chuhaijiang_seller_rank_top_selling.py '<JSON parameters>'           # Automatic: full small results; save large results and print a summary
  python chuhaijiang_seller_rank_top_selling.py '<JSON parameters>' --inline  # Force full output to stdout
...
API_PATH = "/api/v1/tools/research/chuhaijiang/sellers/rankings/top-selling"
...
def main():
    ...
    if not argv:
        print(
            "Usage: chuhaijiang_seller_rank_top_selling.py '<JSON parameters>' [--inline] [--no-cache]",
            file=sys.stderr,
        )
        sys.exit(1)
