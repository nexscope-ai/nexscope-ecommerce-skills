#!/usr/bin/env python3
"""
TikTok Shop Most-Promoted Store Ranking - Nexscope Skill
Call the /api/v1/tools/research/chuhaijiang/sellers/rankings/most-promoted endpoint.

Usage:
  python chuhaijiang_seller_rank_most_promoted.py '<JSON parameters>'           # Automatic: full small results; save large results and print a summary
  python chuhaijiang_seller_rank_most_promoted.py '<JSON parameters>' --inline  # Force full output to stdout
...
API_PATH = "/api/v1/tools/research/chuhaijiang/sellers/rankings/most-promoted"
...
def validate_params(params):
    ...
    rules = {'country': {'type': 'string',
     ...
     'shopType': {'type': 'string', 'enum': ['1', '2', '3', '4']},
...
def main():
    ...
    if not argv:
        print(
            "Usage: chuhaijiang_seller_rank_most_promoted.py '<JSON parameters>' [--inline] [--no-cache]",
            file=sys.stderr,
        )
        sys.exit(1)
