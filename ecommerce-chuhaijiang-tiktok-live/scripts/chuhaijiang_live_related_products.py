#!/usr/bin/env python3
"""
TikTok Live Related Products - Nexscope Skill
Call /api/v1/tools/research/chuhaijiang/lives/related-products endpoint.

Usage:
  python chuhaijiang_live_related_products.py '<JSON parameters>'           # Automatic: full small results; save large results to a file + summary
  python chuhaijiang_live_related_products.py '<JSON parameters>' --inline  # Force complete output to stdout

Output policy (default script behavior):
  - **Always** write the complete response to `<cwd>/nexscope/<YYYY-MM-DD>/<session>/data/ecommerce.chuhaijiang-tiktok-live-<timestamp>.json` (`<cwd>` is the working directory when the script runs, which is the current project directory in Claude Code; `<session>` comes from the `SESSION_ID` environment variable to group outputs automatically by user task; **never write to /tmp**; report an error if the current directory is not writable)
...
API_PATH = "/api/v1/tools/research/chuhaijiang/lives/related-products"
SLUG = "ecommerce.chuhaijiang-tiktok-live"
...
def main():
    ...
    if not argv:
        print(
            "Usage: chuhaijiang_live_related_products.py '<JSON parameters>' [--inline] [--no-cache]",
            file=sys.stderr,
        )
        sys.exit(1)
