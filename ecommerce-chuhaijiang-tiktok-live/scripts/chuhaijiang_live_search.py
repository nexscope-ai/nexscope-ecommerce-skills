#!/usr/bin/env python3
"""
TikTok Live Search - Nexscope Skill
Call /api/v1/tools/research/chuhaijiang/lives/search endpoint.

Usage:
  python chuhaijiang_live_search.py '<JSON parameters>'           # Automatic: full small results; save large results to a file + summary
  python chuhaijiang_live_search.py '<JSON parameters>' --inline  # Force complete output to stdout
...
API_PATH = "/api/v1/tools/research/chuhaijiang/lives/search"
...
def main():
    ...
    if not argv:
        print(
            "Usage: chuhaijiang_live_search.py '<JSON parameters>' [--inline] [--no-cache]",
            file=sys.stderr,
        )
        sys.exit(1)
