#!/usr/bin/env python3
"""
NexScope proxy client for /api/v1/tools/research/ehunt/etsy/storeQuery.

Usage:
  python etsy_store_query.py '<JSON parameters>'
  python etsy_store_query.py '<JSON parameters>' --inline    # force full stdout output
  python etsy_store_query.py '<JSON parameters>' --no-cache  # skip local cache

Output strategy (default script behavior):
  - Always writes full response to <cwd>/nexscope/<YYYY-MM-DD>/<session>/data/ecommerce.etsy-store-query-etsy_store_query-<timestamp>.json
...
API_PATH = '/api/v1/tools/research/ehunt/etsy/storeQuery'
SLUG = 'ecommerce.etsy-store-query-etsy_store_query'
...
def record_output(out_path):
    session_dir = os.path.dirname(os.path.dirname(out_path))
    root = os.path.dirname(os.path.dirname(session_dir))
    meta_path = os.path.join(session_dir, "_meta.json")
    created = not os.path.exists(meta_path)
    try:
        if created:
            meta = {"session_id": os.path.basename(session_dir), "date": os.path.basename(os.path.dirname(session_dir)), "started_at": time.strftime("%Y-%m-%dT%H:%M:%S%z"), "skills_called": [], "deliverables": [], "data_files": [], "media_files": []}
        else:
