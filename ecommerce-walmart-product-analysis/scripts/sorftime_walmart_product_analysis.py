#!/usr/bin/env python3
"""
Walmart Product Analysis - Nexscope Skill
Call the /sorftime/walmart/productAnalysis endpoint.

Usage:
  python sorftime_walmart_product_analysis.py '<JSON parameters>'           # Always save results; print small results in full and summarize large results
  python sorftime_walmart_product_analysis.py '<JSON parameters>' --inline  # Force full output to stdout

Output policy (default script behavior):
  - **Always** save the full response to `<cwd>/nexscope/<YYYY-MM-DD>/<session>/data/nexscope-sorftime-walmart-product-analysis-<timestamp>.json` by default; SESSION_ID groups outputs by user task. If the workspace is not writable, use the fallback roots documented in _nexscope_root().
