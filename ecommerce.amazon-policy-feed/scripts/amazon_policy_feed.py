#!/usr/bin/env python3
"""
NexScope proxy client for /api/v1/tools/research/amazon/policyFeed.

Usage:
  python amazon_policy_feed.py '<JSON parameters>'
  python amazon_policy_feed.py '<JSON parameters>' --inline    # force full stdout output
  python amazon_policy_feed.py '<JSON parameters>' --no-cache  # skip local cache

Output strategy (default script behavior):
  - Always writes full response to <cwd>/nexscope/<YYYY-MM-DD>/<session>/data/ecommerce.amazon-policy-feed-amazon_policy_feed-<timestamp>.json
...
API_PATH = '/api/v1/tools/research/amazon/policyFeed'
SLUG = 'ecommerce.amazon-policy-feed-amazon_policy_feed'
