...
Browse Amazon category nodes by parent identifier or search category metadata by name.
...
|---|---|---:|---:|
| `amazon_category_lookup` | local-script | no | yes |
| `amazon_category_like` | local-script | no | yes |
...
Use `lookup` to browse children of a known node and `like` to search category labels. Read `references/api.md` for marketplace IDs, parameters, and response fields.
