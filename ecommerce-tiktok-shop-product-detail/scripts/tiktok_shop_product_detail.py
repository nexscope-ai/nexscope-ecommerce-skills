#!/usr/bin/env python3
"""
TikTok Shop Product Detail Lookup - Nexscope Skill
Call the /tiktok/shop/product/detail endpoint.

Usage:
  python tiktok_shop_product_detail.py '<JSON parameters>'           # Automatic: print small results in full; save large results and print a summary
  python tiktok_shop_product_detail.py '<JSON parameters>' --inline  # Force full output to stdout

Output policy (default script behavior):
  - **Always** save the full response to `<cwd>/nexscope/<YYYY-MM-DD>/<session>/data/nexscope-tiktok-shop-product-detail-<timestamp>.json` by default; SESSION_ID groups outputs by user task. If the workspace is not writable, use the fallback roots documented in _nexscope_root().
...
API_PATH = "/api/v1/tools/research/tiktok/shop/product/detail"
SLUG = "ecommerce.tiktok-shop-product-detail"
