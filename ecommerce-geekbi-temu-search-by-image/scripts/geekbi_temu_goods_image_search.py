...
Call the /api/v1/tools/research/geekbi/temu/goodsImageSearch endpoint.

Usage:
  python geekbi_temu_goods_image_search.py '<JSON parameters>'
  python geekbi_temu_goods_image_search.py '<JSON parameters>' --inline
...
API_PATH = "/api/v1/tools/research/geekbi/temu/goodsImageSearch"
SLUG = "ecommerce.geekbi-temu-search-by-image"
...
CONTRACT_JSON = """
{
  ...
  "properties": {
    "imageUrl": {
      "type": "string",
      "description": "Provider OSS temporary image URL, obtained through presignedPut and a successful PUT upload with the signature query removed; external URLs are rejected upstream. Actual image must be at most 10 MB and JPEG, PNG, GIF, WebP or BMP.",
      "minLength": 1,
      "maxLength": 2048,
      "format": "uri"
    },
    "contentType": {
      "type": "string",
      "description": "Must match OSS response and actual image bytes; omitted means server detection.",
      "maxLength": 100,
      "enum": [
        "image/jpeg",
        "image/png",
...
def main():
    ...
    if not argv:
        print(
            "Usage: geekbi_temu_goods_image_search.py '<JSON parameters>' [--inline] [--no-cache]",
            file=sys.stderr,
        )
        sys.exit(1)
