...
- **Request URL (Shop Rank)**: `${NEXSCOPE_PROXY_BASE}/api/v1/tools/research/kalodata/shop/rank`
...
| category_ids | array<string> | No | Category ID list filter; results use the selected category dimension |
| revenue_range | string | No | Revenue / GMV range filter |
| shop_type | string | No | Shop type filter: `BRAND` or `RETAILER` |
...
| category_id | string | No | Optional category ID dimension |
| region | string | No | Region/market code, e.g., `US`. Max length 1000 |
...
| Condition | Action |
|---|---|
| Parameter missing/invalid | For example, when `shopId` is missing in detail, returns `msg: shopId is a required parameter`; fix according to `msg` and retry |
...
| Upstream call failed / parameter out of bounds | Two forms: (1) `msg` like `Call to Kalodata API failed: Kalodata API HTTP 554: ` (transient upstream Kalodata error), retry 1-2 times with the same parameters without changing them. (2) `msg` like `page_number range is 1-5, current: 999` (rank parameter out of bounds), fix the parameter and retry |
